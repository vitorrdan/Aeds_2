...
public class Pokemon {
    ...
    static public void imprimir(Pokemon[] array){
    ...
    /*Q02 METODO DE PESQUISA SEQUENCIAL
    Percorre uma lista sequencialmente, se achar elemento correspondente, no caso "name" retorna true*/

   static boolean Pesquisa_Sequencial(ArrayList<Pokemon> lista, String nome){
        boolean resp = false;
   ...
   static void swap(Pokemon[] array, int i, int j){
        Pokemon tmp = array[i];
        array[i] = array[j];
        array[j] = tmp;

    }
    /*Q05 METODO SELECAO POR NAME
     * 
     */

    static void Selecao(Pokemon[] pokemons){
        int n = pokemons.length;
        int count_comp = 0, count_mov = 0;

        for(int i = 0; i < (n - 1); i++){
            int menor = i;
            for(int j = (i + 1); j < n; j++){
                if(pokemons[menor].name.compareTo(pokemons[j].name) > 0 ){
                    count_comp++;
                    menor = j;
                }
            }
    ...
    public static void main(String[] args){
        Pokemon pokemon = new Pokemon();
        List<Pokemon> pokemons = pokemon.LeituraArquivo("pokemon.csv");

        Scanner sc = new Scanner(System.in);

        List<Pokemon> vetorTemp = new ArrayList<>();

        String id; 
        while (true) { //arrumar aq 
            id = sc.nextLine();
            if (id.equals("FIM")) {
                break;
            }
        ...
        // Converte a lista para um array de Pokémons
        Pokemon[] vetor = vetorTemp.toArray(new Pokemon[0]);
